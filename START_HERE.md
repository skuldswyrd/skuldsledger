...
0. **TradingView indicator**: open `~/skuldsledger/indicator/skuld.pine`, paste it into TradingView's Pine Editor, add to chart, and create one alert on "Any alert() function call". Full guide: [indicator/README.md](indicator/README.md). When the repo updates the indicator, re-paste the same file.
