...
- [ ] **(a) Defaults parity.** Same signal labels at the same bars, same cluster lines/labels, same dev levels, same VP, same plan lines/outcome stamps (TP ✓ / SL ✗) as v2.3. New only: DAY TYPE row, APPR/REACT rows (— when idle), approach/REACT labels, feed alerts. SIGNALS counter matches v2.3 bar-for-bar.
...
- [ ] **(k) `stampGateMR` smoke test** (then back OFF): with it ON, a reversal at a cluster with a fresh ✓ REACT still fires; one at an unscored cluster does not; a VWAP/dev-level reversal is unaffected.
